...
interface TypedSSData<T extends string> extends ISpritesheetData {
	animations: Record<T, string[]>
}

const sheet = <T extends string>(meta: Meta, data: SpritesheetData): TypedSSData<T> => {
...
const parse = async <T extends string>(sheet: TypedSSData<T>) => {
...
export const BOW1 = await parse(
	sheet<"draw" | "parry" | "dodge" | "hurt" | "dead">(
		{
