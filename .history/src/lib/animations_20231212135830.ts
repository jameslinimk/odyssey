...
const layers = ["0bas", "1out", "4har", "6tla", "7tlb"] as const
...
const fullSheets = (info: Linfo, { data }: Spritesheet<ISpritesheetData>) => {
	...
	layers.forEach((l) => {
		...
		sheets.push({
			...
			meta: {
				...data.meta,
				image: l === "0bas" ? `${baseFolder}/${baseName}_${type}_v${pl(v)}.png` : `${baseFolder}/${l}/${baseName}_${type}_v${pl(v)}.png`,
			},
		})
	})
	return sheets
